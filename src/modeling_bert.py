...
from src.utils import ALayerNorm
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = ALayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(
        self,
        attribute_embeddings: Optional[torch.Tensor] = None,
        ...
        embeddings = self.LayerNorm(attribute_embeddings ,embeddings)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.Linear_sigma = nn.Linear(config.hidden_size, 1)
        self.Linear_mu = nn.Linear(config.hidden_size, 1)
    ...
    def forward(
        ...
        _, seq_len, hidden_size = mixed_query_layer.shape
        sigma = self.Linear_sigma(attribute_embeddings).view(batch_size, -1, 1, 1)
        mu = self.Linear_mu(attribute_embeddings).view(batch_size, -1, 1, 1)
        mixed_query_layer = mixed_query_layer.view(batch_size, -1, seq_len, hidden_size)
        mixed_query_layer = mixed_query_layer * (1+sigma) + mu
        mixed_query_layer = mixed_query_layer.view(-1, seq_len, hidden_size)
...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, attribute_embeddings, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(attribute_embeddings, hidden_states + input_tensor)
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(attribute_embeddings, self_outputs[0], hidden_states)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attribute_embeddings, attention_output
        )
    ...
    def feed_forward_chunk(self, attribute_embeddings, attention_output):
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(attribute_embeddings, intermediate_output, attention_output)
        return layer_output
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        attributes (`torch.LongTensor` of shape `(batch_size, )`):
            Indices of attribute tokens in the vocabulary.
...
class CrossAttention(nn.Module):
    def __init__(self, 
        ...
        visual_feature_dim = input_size
        text_feature_dim = input_size
        ...
        if cross_attn_type == 0:
            ...
            self._linear_1 = nn.Linear(text_feature_dim, hidden_size)
            self._linear_2 = nn.Linear(visual_feature_dim, hidden_size)
            self._linear_3 = nn.Linear(text_feature_dim + visual_feature_dim, text_feature_dim)
            if use_forget_gate:
                self.fg = nn.Linear(visual_feature_dim + text_feature_dim, visual_feature_dim)
        elif cross_attn_type == 4:
            self._linear_1 = nn.Linear(visual_feature_dim, hidden_size) # K
            self._linear_2 = nn.Linear(visual_feature_dim, hidden_size) # V
            self._linear_3 = nn.Linear(text_feature_dim, hidden_size) # Q
            self._multi_head_attn = nn.MultiheadAttention(hidden_size, n_attn_heads)
            self._linear_4 = nn.Linear(text_feature_dim + hidden_size, text_feature_dim)
            if use_forget_gate:
                self.fg = nn.Linear(hidden_size + text_feature_dim, hidden_size)
        ...
        self.final_layer_norm = nn.LayerNorm(input_size)
        self.sigmoid = nn.Sigmoid()


    def forward(self, image_features, hidden_states):
